...
export default function SafeNavigation() {
  ...
  const calculateRouteWithCoords = async (srcCoords, destCoords) => {
    ...
    try {
      ...
      const routeResults = await evaluateAllRoutes(
        { lat: srcCoords[0], lon: srcCoords[1] },
    ...
    <div className="space-y-10">
      ...
      <div className="grid lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-5 space-y-8">
          ...
          {/* Trusted Places Section UI */}
          <TrustedPlacesSection
            trustedPlaces={trustedPlaces}
            loading={trustedPlacesLoading}
            selectedPlaceId={selectedTrustedPlace?.id}
            onSelectPlace={handleSelectTrustedPlace}
            onAddClick={handleOpenAddModal}
            onEditClick={handleOpenEditModal}
            onDeleteClick={handleDeleteTrustedPlace}
          />

          {/* Selected Trusted Place Details & Nearby Safety Assistance */}
          {selectedTrustedPlace && (
            <TrustedPlaceDetailsCard
              trustedPlace={selectedTrustedPlace}
              onStartNavigation={handleStartNavigationToTrustedPlace}
              onClose={() => setSelectedTrustedPlace(null)}
            />
          )}
